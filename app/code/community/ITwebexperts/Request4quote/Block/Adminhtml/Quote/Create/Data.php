 ...
 */
class ITwebexperts_Request4quote_Block_Adminhtml_Quote_Create_Data extends ITwebexperts_Request4quote_Block_Adminhtml_Quote_Create_Abstract
{
    protected  $_customerCollection = null;

    public function getCustomers(){
        if(is_null($this->_customerCollection)){
            $this->_customerCollection = array();
            $_collection = Mage::getModel('customer/customer')->getCollection()->addAttributeToSelect(array('firstname', 'lastname'))->addOrder('firstname', 'ASC');
            foreach ($_collection as $_customer){
                $this->_customerCollection[$_customer->getId()] = $_customer->getFirstname() . ' ' . $_customer->getLastname();
            }
        }
