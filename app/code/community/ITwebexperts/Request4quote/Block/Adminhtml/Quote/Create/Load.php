 ...
 */
class ITwebexperts_Request4quote_Block_Adminhtml_Quote_Create_Load extends Mage_Core_Block_Template
{
    protected function _toHtml()
    {
        ...
        foreach ($this->getSortedChildren() as $name) {
            if (!$block = $this->getChild($name)) {
                $result[$name] = Mage::helper('request4quote')->__('Invalid block: %s.', $name);
            } else {
