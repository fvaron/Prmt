 ...
 */
class ITwebexperts_Request4quote_Block_Adminhtml_Quote_Create_Search_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
	...
	{
        return Mage::getSingleton('request4quote/adminhtml_session_quote')->getStore();
	}
	...
	protected function _prepareColumns()
	{
		$this->addColumn('entity_id', array(
            'header'    => Mage::helper('request4quote')->__('ID'),
			'sortable'  => true,
			'width'     => '60',
			'index'     => 'entity_id'
		));
		$this->addColumn('name', array(
            'header'    => Mage::helper('request4quote')->__('Product Name'),
			'renderer'  => 'request4quote/adminhtml_quote_create_search_grid_renderer_product',
			'index'     => 'name'
		));
		$this->addColumn('sku', array(
            'header'    => Mage::helper('request4quote')->__('SKU'),
			'width'     => '80',
			'index'     => 'sku'
		));
		...
		$this->addColumn('price', array(
			...
			'index'     => 'price',
            'renderer'  => 'request4quote/adminhtml_quote_create_search_grid_renderer_price',
		));

		$this->addColumn('in_products', array(
            'header'    => Mage::helper('request4quote')->__('Select'),
		...
		$this->addColumn('qty', array(
			'filter'    => false,
			'sortable'  => false,
            'header'    => Mage::helper('request4quote')->__('Qty To Add'),
			'renderer'  => 'request4quote/adminhtml_quote_create_search_grid_renderer_qty',
			'name'      => 'qty',
			'inline_css'=> 'qty',
			'align'     => 'center',
			'type'      => 'input',
		...
		$this->addColumn('currentstock', array(
			'filter'    => false,
			'sortable'  => false,
			'header'    => Mage::helper('sales')->__('Current Stock'),
			'renderer'  => 'request4quote/adminhtml_quote_create_search_grid_renderer_currentstock',
			'name'      => 'currentqty',
		...
		$this->addColumn('remainingstock', array(
			'filter'    => false,
			'sortable'  => false,
			'header'    => Mage::helper('sales')->__('Remaining Stock'),
			'renderer'  => 'request4quote/adminhtml_quote_create_search_grid_renderer_remainingstock',
			'name'      => 'remainingqty',
