 ...
 */
class ITwebexperts_Request4quote_Block_Quote_Totals extends ITwebexperts_Request4quote_Block_Quote_Info_Abstract
{
    /**
    ...
    protected function _initTotals()
    {
        ...
        $i = 0;
        foreach($source->getTotals() as $total => $totalObject){
            if($total == 'shipping') {
                $this->_totals['shipping'] = new Varien_Object(array(
                    'code' => 'shipping',
                    'field' => 'shipping_amount',
                    'value' => $totalObject->getValue(),
                    'label' => $totalObject->getTitle()
                ));
            }
            if($total == 'tax') {
                $this->_totals['tax'] = new Varien_Object(array(
                    'code' => 'tax',
                    'field' => 'tax_amount',
                    'full_info' => $totalObject->getFullInfo(),
                    'value' => $totalObject->getValue(),
                    'label' => $this->__('Tax')
                ));
            }
