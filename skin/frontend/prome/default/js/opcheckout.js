...
Checkout.prototype = {
    initialize: function(accordion, urls){
        ...
        this.steps = ['login', 'billing', 'shipping', 'shipping_method', 'payment'];
        //We use billing as beginning step since progress bar tracks from billing
        this.currentStep = 'billing';
        this.sectionPrefix = 'opc-';
        ...
        this.updateFromAccordion(this.accordion.currentSection);
    },
    ...
    updateFromAccordion: function (section) {
        if(section != "opc-shipping") {
            this.currentStep = section.replace(this.sectionPrefix, '');
            this.refreshToolbarSteps();
        }
    },

    refreshToolbarSteps: function() {
        var currentPosition = sfscStepsConfig[this.currentStep].position;
        // step toolbar
        jQuery.each(sfscStepsConfig, function (i, step) {
            var li = jQuery("[data-checkout-step-code='" + step.code + "']");
            li.removeClass('active');
            li.removeClass('passed');
            if (step.position < currentPosition) {
                li.addClass('passed');
                if (step.back_allowed === true) {
...
Billing.prototype = {
    ...
    newAddress: function(isNew){
        if (isNew) {
           ...
           $$('.billing-new-address-form').each(function(el) {
               el.show();
           });
        } else {
            $$('.billing-new-address-form').each(function(el) {
...
Shipping.prototype = {
    ...
    newAddress: function(isNew){
        if (isNew) {
            this.resetSelectedAddress();
            $$('.shipping-new-address-form').each(function(el) {
                el.show();
            });
        } else {
            $$('.shipping-new-address-form').each(function(el) {
    ...
    init : function () {
        ...
        for (var i=0; i<elements.length; i++) {
            if (elements[i].name=='payment[method]') {
                ...
                /** OVERRIDE start */
            } else if (elements[i].name.indexOf("agreement") == -1) {
                /** OVERRIDE end */
